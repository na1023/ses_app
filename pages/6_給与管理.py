...
from utils.data_manager import load, save, generate_id, init_all, get_company_list
...
with tab_reg:
    ...
    reg_ym = rc1.selectbox(
        "対象月",
        list(reversed(all_ym)),
        format_func=ym_jp,
        key="sal_ym",
    )
    reg_company = rc2.selectbox("会社名", [""] + companies, key="sal_company")
    if not reg_company:
        reg_company = rc2.text_input("会社名（直接入力）", key="sal_company_text", placeholder="株式会社〇〇")
    ...
    if submitted:
        ...
        df_sal = load("salary")
        # 同月同社の重複チェック
        dup = df_sal[(df_sal["year_month"] == reg_ym) & (df_sal["company"] == reg_company)]
        if not dup.empty:
            st.warning(f"{ym_jp(reg_ym)}・{reg_company} の給与はすでに登録されています。一覧タブから編集してください。")
        else:
    ...
    if df_sal.empty:
        ...
        if df_month.empty:
            ...
            for _, row in df_month.iterrows():
                ...
                st.markdown(f"**{row.get('company','—')}**　{ym_jp(sel_ym)}")
...
with tab_summary:
    ...
    st.dataframe(disp[["label","総支給額","控除合計","手取り"]].rename(columns={"label":"月"}),
